...
def unify_string_format(log):
    ...
    # regex that matches strings including nothing but letters and underscores. So, no numbers or other special characters.
...
def split_title_camel_case(log):
    ...
    for col in log.columns:
        ...
        term = [word for word in re.split(r'[.]|(?<=[a-z])(?=[A-Z])|(?<![A-Z])(?=[A-Z][a-z])|(?<=[A-Za-z])(?=[0-9])', col)]
...
def find_event_column(log, ratio_dictionary, action_labels, ratio_threshold):
    ...
    activity_match_count_dictionary, activity_type_dictionary = find_element_types(log, ratio_threshold, ratio_dictionary, action_labels)
    ...
    while True:
        try:
            print(log.head())
            event_column = int(input("Please enter the index of the log's column holding the user interaction assuming that the first column has the index 0:")) 
        except ValueError:
            print("Sorry, this value cannot be accepted. Please enter a number.")
            # return to the start of the loop
            continue
        else:
            if event_column >= 0 and event_column <= len(log.columns):
                # exit the loop.
                break
            else:
                # return to the start of the loop
...
def extract_activity(log, event_column_index):
    ...
    # if the column includes activities instead of events (combination of activity and object type), the object column will be empty
...
# function to count how many unique values there are per column and calculate a ratio (unique_values/total_number_of_values)
...
def rearrange_col_type_dicts(header_obj_types, header_obj_type_from_att_type, column_indices):
    ...
    # concatinate columns where the object type is certain in the ui_obj_att_cols dictionary
...
def get_column_types(log, column_type_dictionary, column_indices, col_compl_dict, ratio_dictionary, threshold_timestamp, threshold_cont_att, threshold_val_att, ui_object_match_count_dictionary, timestamp_match_count_dictionary, url_match_count_dictionary, mail_match_count_dictionary):
    
    # there can only be one 'main ui object type' column holding the ui object interacted with; counter
    main_ui_obj_type_counter = 0

    # we know where the activity column is becaue the 'find_event_column' function puts it in the first position
    ...
    # if there had been an event column, which has been split by the 'extract_activity' function, then the ui object types are in the third position
    ...
    # if urls are found in a column: assume this column includes attribute values and the column header is an attribute type
    ...
    # if email addresses are found in a column: assume this column includes attribute values and the column header is an attribute type
    ...
    for col in column_indices:
        ...
        # ids are usually context attributes even though they have a high uniquness rate
        ...
        # ask user for column type
            ...
            while True:
                ...
                if column_type not in possible_values:
                    print("Column type unvalid. Please try again.")
                    # return to the start of the loop
                    continue
                else:
...
def identify_obj_inst_and_hrchy(log, selected_cols, val_att_cols, cont_att_cols, obj_type_cols, ui_object_type_dictionary, obj_highest_level, obj_second_level, obj_third_level, obj_fourth_level):
